...
const server = http.createServer(async (req, res) => {
  ...
  const { profile_id, gstin, callback_url, callback_token, gst_search_url, headless } = payload;
  if (!gstin) {
    return respond(res, 400, { error: 'Missing gstin in body' });
  }

  // Headless: allow override from request. When Laravel sends headless: false (e.g. client runner),
  // browser opens on the machine where this runner runs. Default to '0' so runner on a client PC shows the browser.
  const headlessEnv = headless === false ? '0' : (headless === true ? '1' : (process.env.HSN_HEADLESS || '0'));
